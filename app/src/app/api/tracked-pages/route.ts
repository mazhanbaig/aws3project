...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error('[TrackedPages GET] Error:', err);
    return NextResponse.json({ error: 'Failed to fetch tracked pages' }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error('[TrackedPages POST] Error:', err);
    return NextResponse.json({ error: 'Failed to create tracked page' }, { status: 500 });
  }
}
